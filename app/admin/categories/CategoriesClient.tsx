...
export default function CategoriesClient({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
      ...
      <section className="rounded-2xl border border-[#A47251]/5 bg-white p-6 sm:p-8 shadow-xs space-y-6">
        <div>
        ...
        <form onSubmit={handleAddCategory} className="flex gap-2">
          <input
            ...
            required
            className="flex-1 bg-[#F0D8A1] border border-[#A47251]/10 rounded-lg p-2.5 text-xs text-[#2A1E17] focus:outline-none focus:border-[#DD9E59]"
          />
          <button
            type="submit"
            disabled={isAddingCat}
            className="bg-[#A47251] text-white rounded-lg px-4 text-xs font-bold uppercase tracking-wider hover:bg-[#DD9E59] hover:text-[#2A1E17] transition-all cursor-pointer disabled:opacity-40"
          >
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="rounded-xl border border-[#A47251]/5 bg-[#F0D8A1]/30 p-4 space-y-3"
            >
              {/* Category Header */}
              ...
              <div className="space-y-2 border-t border-[#A47251]/5 pt-3">
                <span className="block text-[9px] font-bold uppercase tracking-wider text-[#2A1E17]/65">
                  Subcategories
                </span>
                
                {cat.subcategories.length === 0 ? (
                  <span className="text-[10px] text-[#2A1E17]/50 block">No subcategories defined.</span>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {cat.subcategories.map((subcat) => (
                      <span
                        key={subcat}
                        className="inline-flex items-center gap-1 bg-white border border-[#A47251]/5 text-[#2A1E17] rounded-md pl-2 pr-1 py-0.5 text-[10px] font-semibold"
                      >
                        <span>{subcat}</span>
                        <button
                          onClick={() => handleDeleteSubcategory(cat.id, subcat)}
                          aria-label={`Delete ${subcat} subcategory`}
                          className="hover:text-rose-600 text-[#2A1E17]/40 text-xs font-bold transition-colors cursor-pointer"
                        >
                          ×
                        </button>
                      </span>
                ...
                <div className="flex gap-1.5 pt-2">
                  <input
                    ...
                    className="flex-1 bg-white border border-[#A47251]/10 rounded-md p-1.5 text-[10px] text-[#2A1E17] focus:outline-none focus:border-[#DD9E59]"
                  />
                  <button
                    type="button"
                    onClick={() => handleAddSubcategory(cat.id)}
                    className="bg-[#A47251] text-white rounded-md px-2.5 text-[10px] font-bold hover:bg-[#DD9E59] hover:text-[#2A1E17] transition-all cursor-pointer"
                  >
                    +
                  </button>
                </div>
      ...
      <section className="rounded-2xl border border-[#A47251]/5 bg-white p-6 sm:p-8 shadow-xs space-y-6">
        ...
        <div className="space-y-2.5 max-h-[60vh] overflow-y-auto pr-1">
          {badges.map((badge) => (
            <div
              key={badge.id}
              className="flex items-center justify-between rounded-xl border border-[#A47251]/5 bg-[#F0D8A1]/30 px-4 py-3"
            >
              <div className="flex items-center space-x-2">
                <span className="inline-block rounded bg-[#F0D8A1] px-2 py-0.5 text-[10px] font-bold text-white uppercase tracking-wider shadow-sm">
                  {badge.name}
                </span>
                <span className="text-[10px] text-[#2A1E17]/50 font-mono">({badge.id})</span>
              </div>
              <button
