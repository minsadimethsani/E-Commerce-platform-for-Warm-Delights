...
import { Role, PermissionKey, SYSTEM_ROLES } from "@/types/roles";
...
function RolesIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" {...props}>
      <path strokeLinecap="square" strokeLinejoin="miter" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
    </svg>
  );
}
...
const navigation: NavItem[] = [
  { name: "Dashboard", href: "/admin", icon: DashboardIcon, permission: "dashboard.view" },
  { name: "Orders", href: "/admin/orders", icon: OrdersIcon, permission: "orders.view" },
  { name: "Products", href: "/admin/products", icon: ProductsIcon, permission: "products.view" },
  { name: "Optional Add-Ons", href: "/admin/addons", icon: AddOnsIcon, permission: "addons.view" },
  { name: "Categories & Subcategories", href: "/admin/categories", icon: CategoriesIcon, permission: "categories.view" },
  { name: "Reviews", href: "/admin/reviews", icon: ReviewsIcon, permission: "reviews.view" },
  { name: "Role & Access Manager", href: "/admin/roles", icon: RolesIcon, permission: "roles.manage" },
];
...
export default function AdminLayout({
  ...
  // Realtime snapshot listener for roles definitions
  useEffect(() => {
  ...
  if (loading || !user || userProfile?.role === "customer") {
    return (
      <div className="min-h-screen bg-[#FDF9F0] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-[#2A1E17]/60 animate-pulse">Verifying staff credentials...</p>
      </div>
    );
  }

  // Filter allowed navigation items for current user
  const allowedNav = navigation.filter((item) =>
    hasPermission(userProfile?.role, roles, item.permission)
  );

  // Check permission for active route
  const currentNav = navigation.find((item) => item.href === pathname);
  const isAllowedRoute = currentNav
    ? hasPermission(userProfile?.role, roles, currentNav.permission)
    : true;
    ...
    <div className="min-h-screen bg-[#FDF9F0] flex flex-col lg:flex-row">
      ...
      <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 bg-[#A47251] text-[#FDF9F0]/90 border-r border-[#A47251]/10 z-30">
        <div className="flex h-20 items-center px-6 border-b border-[#FDF9F0]/10">
          <Link href="/admin" prefetch={true} className="flex items-center space-x-2">
            ...
            <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-3 mt-2">
              <div className="flex items-center space-x-2.5 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="text-[10px] text-[#DD9E59] font-semibold truncate capitalize">
      ...
      <main className="flex-1 lg:pl-64 min-w-0">
        <div className="px-4 py-8 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          {!isAllowedRoute ? (
            <div className="rounded-2xl border border-rose-200 bg-rose-50/80 p-8 text-center space-y-4 my-8 shadow-xs">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-rose-100 text-rose-600 font-bold text-xl">
                !
              </div>
              <h2 className="font-serif text-2xl font-bold text-rose-900">Access Restricted</h2>
              <p className="text-sm text-rose-700 max-w-md mx-auto leading-relaxed">
                Your assigned staff role (<span className="font-bold">{userProfile?.role}</span>) does not have permission to view or manage this section (<span className="font-mono text-xs">{pathname}</span>). Please contact a Super Admin to update your access role.
              </p>
              <div className="pt-2">
                <Link
                  href="/admin"
                  className="inline-block rounded-full bg-[#A47251] text-white px-6 py-2.5 text-xs font-bold uppercase tracking-wider hover:bg-[#DD9E59] hover:text-[#2A1E17] transition-all shadow-xs"
                >
                  Return to Dashboard
                </Link>
              </div>
